...
df2 = df.rename(columns = {'identifier' :'Subject', 'file_name' :'filename', 'original_file_name' :'Title', 'date_created':'Date',  'google_id' :'Source', 'google_parent_id' :'Relation', 'mimeType' :'Type', 'size' :'Coverage', 'owners' :'Creator', 'lastModifyingUser' :'Contributor', 'ID' :'Identifier'}, inplace = False)
df2.head()

#list columns
df2.columns = ['Subject', 'filename', 'description', 'Title', 'folder',	'Date', 'date_last_modified', 'checksum_md5', 'closure_type', 'closure_period', 'closure_start_date', 'foi_exemption_code', 'foi_exemption_asserted', 'title_public', 'title_alternate', 'description_public', 'description_alternate', 'Source', 'Relation', 'Rights', 'Publisher', 'Type', 'Coverage', 'archivist_note', 'file_name_note', 'original_identifier', 'Language', 'sharingUser', 'Creator', 'Contributor', 'Identifier']
...
df2['Description'] = df2['Description'].str.replace("nan","", regex=True).str.replace("nan nan","", regex=True)
df2['Date'] = df2['Date'].apply(str) + '; modified ' + df2['date_last_modified'].apply(str)
...
df2 = df2.drop(['description', 'folder', 'date_last_modified', 'checksum_md5', 'archivist_note', 'file_name_note', 'sharingUser', 'closure_type', 'closure_period', 'closure_start_date', 'foi_exemption_code', 'foi_exemption_asserted', 'original_identifier', 'title_public', 'title_alternate', 'description_public', 'description_alternate'], axis = 1, inplace = False)
